...
function optimizeContentForSEO(content, serviceName, location, clusterKeywords) {
  ...
  // Step 1: Clean up content (remove editor metadata, page builder code, preserve valuable info)
  let cleanedContent = content
    ...
    // Remove Elementor page builder attributes
    .replace(/\s*data-elementor-type="[^"]*"/g, '')
    .replace(/\s*data-elementor-id="[^"]*"/g, '')
    .replace(/\s*data-elementor-post-type="[^"]*"/g, '')
    .replace(/\s*data-elementor-settings="[^"]*"/g, '')
    .replace(/\s*data-elementor-type="[^"]*"/g, '')
    .replace(/\s*data-id="[^"]*"/g, '')
    ...
    .replace(/\s*class="e-con[^"]*"/g, '')
    .replace(/\s*class="e-flex[^"]*"/g, '')
    .replace(/\s*class="cmsmasters[^"]*"/g, '')
    // Remove empty divs that only had Elementor classes
